...
const SocialPlatformConfigs = {
  ...
  bilibili: {Icon: Bilibili, label: '哔哩哔哩'},
  douyin: {Icon: douyin, label: '抖音'},
};
...
function Bilibili({className}) {
  return (
    <i
      className={clsx('fa-brands', 'fa-bilibili', className, styles.bilibiliIcon)}
      aria-hidden="true"
    />
  );
}
function douyin({className}) {
  return (
    <i
      className={clsx('fa-brands', 'fa-tiktok', className, styles.douyincon)}
      aria-hidden="true"
    />
  );
}